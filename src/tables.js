...
Table.prototype.toList = function () {
  return new List(this.contents.map((eachRow) => new List(eachRow)));
};
...
TableCellMorph.prototype.listSymbol = function () {
  if (
    !this.cachedListSymbol ||
    this.cachedListSymbol.height() !== SyntaxElementMorph.prototype.fontSize
  ) {
...
TableCellMorph.prototype.render = function (ctx) {
  var dta = this.labelString || this.dataRepresentation(this.data),
    ...
    empty = TableMorph.prototype.highContrast
      ? "rgb(220, 220, 220)"
      : "transparent",
  ...
  if (this.shouldBeList()) {
    BoxMorph.prototype.outlinePath.call(
      this,
      ctx,
      SyntaxElementMorph.prototype.corner + 1,
      0
    );
    ctx.fill();
...
TableCellMorph.prototype.selectContextForEdit = function () {
  var script = this.data.toBlock(),
    ...
    ide = this.parentThatIsA(IDE_Morph) || this.world().childThatIsA(IDE_Morph);
  ...
  for (c = 0; c <= this.columns.length; c += 1) {
    for (r = 0; r <= this.rows; r += 1) {
      cell = new TableCellMorph(
        ...
        new Point(
          !c ? this.rowLabelWidth : this.colWidth(c + this.startCol - 1),
      ...
      cell.setPosition(
        new Point(
          !c ? this.padding : this.columns[c - 1],
          !r
            ? this.padding
            : this.padding * 2 +
              this.colLabelHeight +
...
TableMorph.prototype.scroll = function (xSteps, ySteps) {
  this.showData(
    Math.min(this.maxStartCol, Math.max(1, this.startCol + Math.round(xSteps))),
    Math.min(this.maxStartRow, Math.max(1, this.startRow + Math.round(ySteps)))
  );
  this.updateScrollBars();
};
  ...
  return (
    Math.max(
      ...
      Math.max.apply(
        ...
        this.table
          .columnNames()
          .map((name) => (name ? ctx.measureText(name).width : 0))
      )
    ) ||
...
TableMorph.prototype.resizeCells = function (pos) {
  ...
  if (this.resizeCol) {
    this.colWidths[this.resizeCol - 1] = Math.max(
      16,
      (this.colWidths[this.resizeCol - 1] || this.globalColWidth) + delta.x
    );
    ...
    for (i = 0; i < this.colWidths.length; i += 1) {
      if (this.colWidths[i]) {
        this.colWidths[i] = Math.max(16, this.colWidths[i] + delta.x);
      }
    }
  }
  this.rowLabelWidth = this.rowLabelsWidth();
...
TableDialogMorph.prototype.setInitialDimensions = function () {
  ...
  this.setExtent(
    this.tableView
      .globalExtent()
      .add(new Point(this.padding * 2, this.padding * 2 + th + bh))
      .min(mex)
...
TableDialogMorph.prototype.fixLayout = BlockEditorMorph.prototype.fixLayout;
