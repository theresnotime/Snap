...
SymbolMorph.prototype.renderSymbolGearBig = function (ctx, color) {
  ...
  angle = 360 / spikes;
  for (i = 0; i < spikes; i += 1) {
    ctx.arc(r, r, r, radians(i * angle), radians(i * angle + off));
    ctx.arc(
      r,
      r,
      r * 0.8,
    ...
    ctx.arc(r, r, r, radians((i + 1) * angle - off), radians((i + 1) * angle));
  }
  ctx.lineTo(w, r);
...
SymbolMorph.prototype.renderSymbolFlag = function (ctx, color) {
  ...
  ctx.beginPath();
  ctx.moveTo(0, h / 4);
  ctx.bezierCurveTo(w * 0.8, h / 4, w * 0.1, h * 0.5, w, h * 0.5);
  ctx.stroke();
};
...
SymbolMorph.prototype.renderSymbolStorage = function (ctx, color) {
  ...
  function drawDisk(bottom) {
    ...
    ctx.arc(w / 2, bottom - h - unit * 2, r, radians(120), radians(60), true);
    ctx.closePath();
    ctx.fill();
...
SymbolMorph.prototype.renderSymbolSpeechBubbleOutline = function (ctx, color) {
  // draw a speech bubble
  var w = this.symbolWidth(),
    h = this.size,
    n = w / 3,
...
SymbolMorph.prototype.renderSymbolMagnifyingGlass = function (ctx, color) {
  ...
  gradient = ctx.createRadialGradient(x, y, 0, x + r, y + r, w);
...
SymbolMorph.prototype.renderSymbolKeyboard = function (ctx, color) {
  ...
  for (row = 0; row < 2; row += 1) {
    for (col = 0; col < 5; col += 1) {
      ctx.fillRect((u + k) * col + u, (u + k) * row + u, k, k);
    }
  }
...
SymbolMorph.prototype.renderSymbolKeyboardFilled = function (ctx, color) {
  ...
  for (row = 0; row < 2; row += 1) {
    for (col = 0; col < 5; col += 1) {
      ctx.rect((u + k) * col + u, (u + k) * row + u, k, k);
    }
  }
...
SymbolMorph.prototype.renderSymbolList = function (ctx, color) {
  ...
  for (row = 0; row < 4; row += 1) {
    ctx.rect(padding, (padding + item) * row + padding, w - item, item);
  }
  ctx.clip("evenodd");
  ctx.fillRect(0, 0, w, h);
};
