...
IDE_Morph.prototype.switchTo = function (sceneName) {
  const scene = detect(
    this.scenes.itemsArray(),
    (scn) => scn.name === sceneName
  );
  if (scene === null) {
...
IDE_Morph.prototype.broadcast = function (message, callback) {
  ...
  const rcvrs = this.sprites.contents.concat(this.stage);
  const myself = this;
  const procs = [];
  ...
  (this.stage.messageCallbacks[""] || []).forEach((callback) =>
    callback(message)
  );
  (this.stage.messageCallbacks[message] || []).forEach((callback) =>
    callback()
  );
};
