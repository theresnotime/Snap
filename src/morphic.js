...
function embedMetadataPNG(aCanvas, aString) {
  var embedTag = MorphicPreferences.pngPayloadMarker,
    ...
    int2BStr = (val) =>
      arr2Str(
        Array.from(new Uint8Array(new Uint32Array([val]).buffer)).reverse()
      ),
    buildChunk = (data) => {
      let res = "iTXt" + data;
    ...
    newChunk = buildChunk(
      "Snap!_SRC\0\0\0\0\0" + embedTag + encodeURIComponent(aString) + embedTag
    );
...
*/
  ...
  var ctx = document.createElement("canvas").getContext("2d"),
    ...
    uber = {
      ...
      width: Object.getOwnPropertyDescriptor(canvasProto, "width"),
      height: Object.getOwnPropertyDescriptor(canvasProto, "height"),
      ...
      shadowBlur: Object.getOwnPropertyDescriptor(contextProto, "shadowBlur"),
    };
  ...
  function getPixelRatio(imageSource) {
    return imageSource.isRetinaEnabled
      ? (originalDevicePixelRatio || 1) / backingStorePixelRatio
      : 1;
  }
  ...
  Object.defineProperty(canvasProto, "width", {
    ...
    set: function (width) {
      try {
        // workaround one of FF's dreaded NS_ERROR_FAILURE bugs
  ...
  contextProto.drawImage = function (image) {
    ...
    switch (arguments.length) {
      ...
      default:
        throw Error(
          "Called drawImage() with " + arguments.length + " arguments"
        );
    }
    uber.drawImage.call(
      this,
  ...
  Object.defineProperty(contextProto, "shadowOffsetX", {
    get: function () {
      return uber.shadowOffsetX.get.call(this) / getPixelRatio(this.canvas);
    },
    set: function (offset) {
      var pixelRatio = getPixelRatio(this.canvas);
  ...
  Object.defineProperty(contextProto, "shadowOffsetY", {
    get: function () {
      return uber.shadowOffsetY.get.call(this) / getPixelRatio(this.canvas);
    },
    set: function (offset) {
      var pixelRatio = getPixelRatio(this.canvas);
  ...
  Object.defineProperty(contextProto, "shadowBlur", {
    get: function () {
      return uber.shadowBlur.get.call(this) / getPixelRatio(this.canvas);
    },
    set: function (blur) {
      var pixelRatio = getPixelRatio(this.canvas);
...
function isRetinaSupported() {
  ...
  return (
    backingStorePixelRatio !== window.devicePixelRatio &&
    !Object.keys(uber).some((any) => {
      var prop = uber[any];
      return prop.hasOwnProperty("configurable") && !prop.configurable;
    })
...
Rectangle.prototype.regionsAround = function (aRect) {
  ...
  if (aRect.left() > this.left()) {
    regions.push(
      new Rectangle(this.left(), this.top(), aRect.left(), this.bottom())
    );
  }
  // above:
  if (aRect.top() > this.top()) {
    regions.push(
      new Rectangle(this.left(), this.top(), this.right(), aRect.top())
    );
  }
  // right:
  if (aRect.right() < this.right()) {
    regions.push(
      new Rectangle(aRect.right(), this.top(), this.right(), this.bottom())
    );
  }
  // below:
  if (aRect.bottom() < this.bottom()) {
    regions.push(
      new Rectangle(this.left(), aRect.bottom(), this.right(), this.bottom())
    );
  }
  return regions;
};
...
Morph.prototype.setFullCenter = function (aPoint) {
  this.setPosition(
    aPoint.subtract(this.fullBounds().extent().floorDivideBy(2))
  );
};
...
Morph.prototype.drawOn = function (ctx, rect) {
  ...
  if (this.isCachingImage) {
    ...
    ctx.drawImage(pic, sl, st, w, h, clipped.left(), clipped.top(), w, h);
  } else {
    // render directly on target canvas
    ctx.beginPath();
    ...
    if (MorphicPreferences.showHoles) {
      ...
      this.holes.forEach((hole) => {
        ...
        ctx.fillRect(sect.left(), sect.top(), sect.width(), sect.height());
      });
    }
  }
  ctx.restore();
...
Morph.prototype.shadowImage = function (off, color) {
  ...
  if (this.fullShadowSource) {
    ...
    // optimization when all submorphs are contained inside
    fb = this.extent();
    img = this.getImage();
  }
...
Morph.prototype.getShadow = function () {
  var shadows;
  shadows = this.children
    .slice(0)
    .reverse()
    .filter((child) => child instanceof ShadowMorph);
  if (shadows.length !== 0) {
    return shadows[0];
  }
  return null;
...
Morph.prototype.topMorphAt = function (point) {
  ...
  if (this.bounds.containsPoint(point)) {
    if (
      this.holes.some((any) =>
        any.translateBy(this.position()).containsPoint(point)
      )
    ) {
      return null;
    }
...
Morph.prototype.pickUp = function (wrrld) {
  ...
  this.setPosition(
    world.hand.position().subtract(this.extent().floorDivideBy(2))
  );
...
Morph.prototype.perish = function (msecs, onComplete) {
  this.fadeTo(0, msecs === 0 ? 0 : msecs || 100, null, () => {
    this.destroy();
    if (onComplete) {
      onComplete();
    }
...
Morph.prototype.inspect = function (anotherObject) {
  var world =
      this.world instanceof Function ? this.world() : this.root() || this.world,
    inspector,
    inspectee = this;
...
Morph.prototype.hierarchyMenu = function () {
  ...
  parents.forEach((each) => {
    if (each.developersMenu && each !== world) {
      menu.addMenu(each.toString().slice(0, 50), each.developersMenu());
    }
  });
  return menu;
};
...
Morph.prototype.developersMenu = function () {
  ...
  menu.addItem("pick up", "pickUp", "detach and put \ninto the hand");
...
Morph.prototype.allEntryFields = function () {
  return this.allChildren().filter((each) => {
    return (
      each.isEditable &&
      (each instanceof StringMorph || each instanceof TextMorph)
    );
  });
};
...
HandleMorph.prototype.fixLayout = function () {
  if (this.target) {
    if (this.type === "moveCenter") {
      ...
      // 'resize', 'move'
      this.setPosition(
        this.target.bottomRight().subtract(this.extent().add(this.inset))
      );
    }
...
HandleMorph.prototype.renderHandleOn = function (ctx, color, shadowColor) {
...
ColorPickerMorph.prototype.fixLayout = function () {
  ...
  y =
    gpal.bottom() +
    Math.floor((this.bottom() - gpal.bottom() - this.feedback.height()) / 2);
...
SpeechBubbleMorph.prototype.fixLayout = function () {
  ...
  if (this.contents instanceof Morph) {
    ...
    this.contentsMorph.setExtent(
      new Point(this.contents.width, this.contents.height)
    );
...
SpeechBubbleMorph.prototype.outlinePath = function (ctx, radius, inset) {
  ...
  if (!this.isThought) {
    ...
    if (this.isPointingRight) {
      ctx.lineTo(offset + radius, h - offset);
      ctx.lineTo(radius / 2 + inset, h - inset);
    } else {
      // pointing left
      ctx.lineTo(w - (radius / 2 + inset), h - inset);
      ctx.lineTo(w - (offset + radius), h - offset);
    }
  }
  // bottom left:
  ctx.arc(
...
DialMorph.prototype.render = function (ctx) {
  ...
  ctx.fillStyle = this.color.toString();
  ctx.beginPath();
  ctx.arc(this.radius, this.radius, face, 0, 2 * Math.PI, false);
  ctx.closePath();
  ctx.fill();
  ...
  ctx.beginPath();
  ctx.arc(this.radius, this.radius, face, Math.PI / -2, angle, false);
  ...
  ctx.fillStyle = "black";
  ctx.beginPath();
  ctx.arc(this.radius, this.radius, inner, 0, 2 * Math.PI, false);
  ctx.closePath();
  ctx.fill();
  ...
  ctx.fillStyle = "black";
  ctx.beginPath();
  ctx.arc(x2, y2, inner, 0, 2 * Math.PI, false);
  ctx.closePath();
  ctx.stroke();
...
CircleBoxMorph.prototype.render = function (ctx) {
  ...
  if (this.orientation === "vertical") {
    ...
    // top semi-circle
    ctx.arc(radius, radius, radius, radians(180), radians(0), false);

    // right line
    ctx.lineTo(w, h - radius);

    // bottom semi-circle
    ctx.arc(radius, h - radius, radius, radians(0), radians(180), false);
  } else {
    radius = h / 2;
    ctx.beginPath();

    // left semi-circle
    ctx.arc(radius, radius, radius, radians(90), radians(-90), false);

    // top line
    ctx.lineTo(w - radius, 0);

    // right semi-circle
    ctx.arc(w - radius, radius, radius, radians(-90), radians(90), false);
  }
  ctx.closePath();
  ctx.fillStyle = this.color.toString();
  ctx.fill();
...
SliderButtonMorph.prototype.renderEdges = function (ctx) {
  ...
  if (this.orientation === "vertical") {
    ctx.lineWidth = w / 3;
    gradient = ctx.createLinearGradient(0, 0, ctx.lineWidth, 0);
    ...
    gradient = ctx.createLinearGradient(w - ctx.lineWidth, 0, w, 0);
    ...
    if (this.hasMiddleDip) {
      ...
      ctx.fillStyle = gradient;
      ctx.beginPath();
      ctx.arc(w / 2, h / 2, radius, radians(0), radians(360), false);
      ctx.closePath();
      ctx.fill();
    }
  } else if (this.orientation === "horizontal") {
    ctx.lineWidth = h / 3;
    gradient = ctx.createLinearGradient(0, 0, 0, ctx.lineWidth);
    gradient.addColorStop(0, "white");
    gradient.addColorStop(1, this.color.toString());
    ...
    gradient = ctx.createLinearGradient(0, h - ctx.lineWidth, 0, h);
    gradient.addColorStop(0, this.color.toString());
    gradient.addColorStop(1, "black");
...
SliderMorph.prototype.setStart = function (num, noUpdate) {
  ...
  if (typeof num === "number") {
    this.start = Math.min(num, this.stop - this.size);
  } else {
    newStart = parseFloat(num);
    if (!isNaN(newStart)) {
      this.start = Math.min(newStart, this.stop - this.size);
    }
  }
...
InspectorMorph.prototype.buildPanes = function () {
  ...
  if (this.showing === "attributes") {
    attribs = attribs.filter((prop) => typeof this.target[prop] !== "function");
  ...
  doubleClickAction = () => {
    ...
    world = this.world();
    inspector = new InspectorMorph(this.currentProperty);
    inspector.setPosition(world.hand.position());
    inspector.keepWithin(world);
    world.add(inspector);
    inspector.changed();
    ...
    this.markOwnProperties
      ? [
          ...
          [
            ...
            new Color(0, 0, 180),
            (element) => {
              return Object.prototype.hasOwnProperty.call(this.target, element);
            },
          ],
        ]
      : null,
  ...
  this.buttonSubset.action = () => {
    ...
    menu.addLine();
    menu.addItem(
      this.markOwnProperties ? "un-mark own" : "mark own",
      () => {
  ...
  this.buttonInspect.action = () => {
    ...
    if (isObject(this.currentProperty)) {
      ...
      this.inform(
        (this.currentProperty === null ? "null" : typeof this.currentProperty) +
          "\nis not inspectable"
      );
    }
  };
...
MenuMorph.prototype.createItems = function () {
  ...
  this.items.forEach((tuple) => {
    isLine = false;
    if (
      tuple instanceof StringFieldMorph ||
...
MenuMorph.prototype.maxWidth = function () {
  ...
  this.children.forEach((item) => {
    if (item instanceof MenuItemMorph) {
      w = Math.max(
        w,
        item.label.width() + 8 + (item.shortcut ? item.shortcut.width() + 4 : 0)
      );
    } else if (
  ...
  this.children.forEach((item) => {
    ...
    if (item === this.label) {
      item.text.setPosition(
        item.center().subtract(item.text.extent().floorDivideBy(2))
      );
    }
  });
};
  ...
  this.children.forEach((item) => {
    if (item instanceof MenuItemMorph) {
      ...
      item.contents.children.forEach((morph) => {
        if (morph instanceof MenuItemMorph && morph.userState !== "normal") {
...
MenuMorph.prototype.popUpCenteredAtHand = function (world) {
  ...
  this.popup(
    wrrld,
    wrrld.hand.position().subtract(this.extent().floorDivideBy(2))
  );
};
...
StringMorph.prototype.fixLayout = function (justMe) {
  ...
  var width,
    ...
    txt = this.isPassword ? this.password("*", this.text.length) : this.text;
  ...
  this.bounds.corner = this.bounds.origin.add(
    new Point(width, fontHeight(this.fontSize) + Math.abs(shadowOffset.y))
  );
...
StringMorph.prototype.render = function (ctx) {
  ...
  if (this.isShowingBlanks) {
    this.renderWithBlanks(ctx, x, fontHeight(this.fontSize) + y);
  } else {
    ctx.fillText(txt, x, fontHeight(this.fontSize) + y);
  }
...
StringMorph.prototype.renderWithBlanks = function (ctx, startX, y) {
  ...
  function drawBlank() {
    ...
    ctx.beginPath();
    ctx.arc(x + space / 2, top, space / 2, radians(0), radians(360));
    ctx.fill();
    x += space;
  }
...
StringMorph.prototype.slotPosition = function (slot) {
  ...
  var txt = this.isPassword ? this.password("*", this.text.length) : this.text,
...
StringMorph.prototype.slotAt = function (aPoint) {
  ...
  while (aPoint.x - this.left() > charX) {
    ...
    if (idx === txt.length) {
      if (
        this.measureCtx.measureText(txt).width -
...
TriggerMorph.prototype.fixLayout = function () {
  this.label.setPosition(
    this.center().subtract(this.label.extent().floorDivideBy(2))
  );
};
...
TriggerMorph.prototype.triggerDoubleClick = function () {
  ...
  if (typeof this.target === "function") {
    if (typeof this.doubleClickAction === "function") {
      this.target.call(this.environment, this.doubleClickAction.call(), this);
    } else {
...
TriggerMorph.prototype.bubbleHelp = function (contents) {
  var world = this.world();
  this.schedule = new Animation(nop, nop, 0, 500, nop, () =>
...
TriggerMorph.prototype.popUpbubbleHelp = function (contents) {
  new SpeechBubbleMorph(localize(contents), null, null, 1).popUp(
    this.world(),
    this.rightCenter().add(new Point(-8, 0))
  );
};
  ...
  if (this.scrollFrame.isTextLineWrapping) {
    this.children.forEach((morph) => {
      if (morph instanceof TextMorph) {
        ...
        this.setHeight(Math.max(morph.height(), this.scrollFrame.height()));
      }
    });
  }
...
ScrollFrameMorph.prototype.adjustScrollBars = function () {
  ...
  if (this.contents.height() > this.height()) {
    ...
    this.vBar.setPosition(
      new Point(this.right() - this.vBar.width(), this.top())
    );
    this.vBar.start = 0;
    this.vBar.stop =
...
ScrollFrameMorph.prototype.autoScroll = function (pos) {
  ...
  area = new Point(this.right() - inset, this.top()).extent(
    new Point(inset, this.height())
  );
  if (area.containsPoint(pos)) {
    this.scrollX(-(inset - (this.right() - pos.x)));
  }
  area = new Point(this.left(), this.bottom() - inset).extent(
...
StringFieldMorph.prototype.createText = function () {
  ...
  this.setExtent(
    new Point(Math.max(this.width(), this.minWidth), this.text.height())
  );
  this.add(this.text);
};
...
BouncerMorph.prototype.step = function () {
  if (!this.isStopped) {
    if (this.type === "vertical") {
      ...
      if (
        this.fullBounds().top() < this.parent.top() &&
        this.direction === "up"
      ) {
        this.direction = "down";
      }
      if (
        this.fullBounds().bottom() > this.parent.bottom() &&
        this.direction === "down"
      ) {
        this.direction = "up";
      }
      ...
      if (
        this.fullBounds().left() < this.parent.left() &&
        this.direction === "left"
      ) {
        this.direction = "right";
      }
      if (
        this.fullBounds().right() > this.parent.right() &&
        this.direction === "right"
      ) {
        this.direction = "left";
      }
...
HandMorph.prototype.processMouseMove = function (event) {
  ...
  mouseOverBoundsNew = mouseOverNew.filter(
    (m) =>
      m.isVisible &&
      m.visibleBounds().containsPoint(this.bounds.origin) &&
      !m.holes.some((any) =>
  ...
  mouseOverNew.forEach((newMorph) => {
    ...
    if (this.children.length > 0) {
      if (
        newMorph instanceof ScrollFrameMorph &&
        ...
        if (
          !newMorph.bounds
            .insetBy(MorphicPreferences.scrollBarSize * 3)
...
HandMorph.prototype.processMouseScroll = function (event) {
  ...
  if (morph) {
    morph.mouseScroll(
      event.detail / -3 ||
        (Object.prototype.hasOwnProperty.call(event, "wheelDeltaY")
    ...
    pic.onload = () => {
      (async () => {
        ...
        if (strBuff.includes(embedTag)) {
          try {
            embedded = decodeURIComponent(strBuff?.split(embedTag)[1]);
          } catch (err) {
            console.log(err);
          }
        }
  ...
  if (files.length > 0) {
    ...
    for (i = 0; i < files.length; i += 1) {
      ...
      if (
        ...
        // assume it's meant to be binary
        readBinary(file);
      }
    }
  } else if (url) {
    suffix = url.slice(url.lastIndexOf(".") + 1).toLowerCase();
    if (contains(["gif", "png", "jpg", "jpeg", "bmp"], suffix)) {
...
*/
  this.temporaries.forEach((morph) => {
    if (!(morph.isClickable && morph.bounds.containsPoint(this.position()))) {
      morph.destroy();
...
WorldMorph.prototype.initKeyboardHandler = function () {
  ...
  if (kbd) {
    // share existing handler with other worlds
  ...
  kbd.addEventListener(
    ...
    (event) => {
      ...
      if (kbd.world.keyboardFocus && kbd.world.keyboardFocus.processKeyDown) {
      ...
      if ((event.ctrlKey || event.metaKey) && "dfiops".includes(event.key)) {
        event.preventDefault();
      }
    },
    true
  ...
  kbd.addEventListener(
    ...
    (event) => {
      ...
      if (kbd.world.keyboardFocus && kbd.world.keyboardFocus.processKeyUp) {
  ...
  kbd.addEventListener(
    "keypress",
    (event) => {
      if (kbd.world.keyboardFocus && kbd.world.keyboardFocus.processKeyPress) {
        kbd.world.keyboardFocus.processKeyPress(event);
        event.preventDefault();
      }
    },
  ...
  kbd.addEventListener(
    "input",
    (event) => {
      if (kbd.world.keyboardFocus && kbd.world.keyboardFocus.processInput) {
        // flush the world's currentKey property
        kbd.world.currentKey = null;
...
WorldMorph.prototype.slide = function (aStringOrTextMorph) {
  ...
  slider.action = (num) => {
    ...
    aStringOrTextMorph.escalateEvent("reactToSliderEdit", aStringOrTextMorph);
  };
  menu.items.push(slider);
