...
SpriteMorph.prototype.isDisablingBlock = function (aBlock) {
  ...
  if (sel === "doApplyExtension" || sel === "reportApplyExtension") {
  ...
  var stage = this.parentThatIsA(StageMorph),
    sound =
      name instanceof Sound
        ...
        : typeof name === "number"
        ? this.sounds.at(name)
        : detect(this.sounds.asArray(), (s) => s.name === name.toString()),
...
SpriteMorph.prototype.hasGenericHatBlocks = function () {
  return this.scripts.children.some(
    (morph) => morph.selector === "receiveCondition"
  );
};
...
SpriteMorph.prototype.allLocalVariableNames = function (sorted, all) {
  ...
  data = this.variables
    .allNames(exceptGlobals, all)
    .filter((each) => !contains(globalNames, each));
  if (sorted) {
...
SpriteMorph.prototype.reachableGlobalVariableNames = function (sorted, all) {
  ...
  data = this.globalVariables()
    .names(all)
    .filter((each) => !contains(locals, each));
  if (sorted) {
    data.sort(alphabetically);
  }
  return data;
...
StageMorph.prototype.fireGreenFlagEvent = function () {
  ...
  this.children.concat(this).forEach((morph) => {
    if (isSnapObject(morph)) {
      morph
        .allHatBlocksFor("__shout__go__")
        .forEach((block) =>
          procs.push(this.threads.startProcess(block, morph, this.isThreadSafe))
        );
    }
  });
  if (ide) {
...
StagePrompterMorph.prototype.fixLayout = function () {
  ...
  this.inputField.setWidth(
    this.width() - this.edge * 2 - this.button.width() - this.border
  );
