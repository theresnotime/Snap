...
PushButtonMorph.prototype.drawOutline = function (ctx) {
  ...
  ctx.beginPath();
  this.outlinePath(ctx, isFlat ? 0 : this.corner, 0);
  ctx.closePath();
  ctx.fill();
};
...
DialogBoxMorph.prototype.promptRGB = function (title, color, world, pic, msg) {
...
DialogBoxMorph.prototype.outlinePathTitle = function (ctx, radius) {
  ...
  // top left:
  ctx.arc(radius, radius, radius, radians(-180), radians(-90), false);
  // top right:
  ctx.arc(w - radius, radius, radius, radians(-90), radians(-0), false);
  // bottom right:
  ctx.lineTo(w, h);
...
DialogBoxMorph.prototype.outlinePathBody = function (ctx, radius) {
  ...
  // bottom right:
  ctx.arc(w - radius, h - radius, radius, radians(0), radians(90), false);
  // bottom left:
  ctx.arc(radius, h - radius, radius, radians(90), radians(180), false);
};
...
PianoMenuMorph.prototype.selectKey = function (midiNum) {
  ...
  key = detect(this.children, (each) => each.action === midiNum);
  if (key) {
    this.select(key);
  } else {
    this.selectKey(48);
