...
CustomBlockDefinition.prototype.blockInstance = function (storeTranslations) {
  ...
  if (this.type === "command") {
    ...
    block = new CustomReporterBlockMorph(this, this.type === "predicate");
  }
...
CustomBlockDefinition.prototype.prototypeInstance = function () {
  ...
  if (this.type === "command") {
    ...
    block = new CustomReporterBlockMorph(this, this.type === "predicate", true);
  }
...
CustomBlockDefinition.prototype.copyAndBindTo = function (sprite, headerOnly) {
  ...
  if (headerOnly) {
    // for serializing inherited method signatures
    c.body = null;
    return c;
  }
  if (c.body) {
...
CustomBlockDefinition.prototype.dropDownMenuOf = function (inputName) {
  var fname;
  if (this.declarations.has(inputName) && this.declarations.get(inputName)[2]) {
...
CustomBlockDefinition.prototype.menuSearchWords = function () {
  ...
  this.inputNames().forEach((slot) => {
    ...
    if (menu) {
      if (isString(menu)) {
        // special menu, translates its values
        ...
        // assume a dictionary, take its keys
...
CustomBlockDefinition.prototype.isReadOnlyInput = function (inputName) {
  return (
    this.declarations.has(inputName) &&
...
CustomBlockDefinition.prototype.isDirectlyRecursive = function () {
  ...
  if (!this.body) {
    ...
    this.cachedIsRecursive = this.body.expression.anyChild(function (morph) {
      return morph.isCustomBlock && morph.blockSpec === myspec;
    });
  }
  return this.cachedIsRecursive;
};
...
CustomBlockDefinition.prototype.abstractBlockSpec = function () {
  ...
  // with an underscore
  return BlockMorph.prototype
    .parseSpec(this.blockSpec())
...
CustomBlockDefinition.prototype.translationsAsText = function () {
  var txt = "";
  Object.keys(this.translations).forEach(
    (lang) => (txt += lang + ":" + this.translations[lang] + "\n")
  );
  return txt;
};
...
CustomBlockDefinition.prototype.setBlockLabel = function (abstractSpec) {
  ...
  parts.forEach(
    (part, i) =>
      (spec += part + (inputNames[i] ? " %'" + inputNames[i] + "' " : ""))
  );
  this.spec = spec.trim();
};
...
CustomBlockDefinition.prototype.setBlockDefinition = function (aContext) {
  ...
  parts.forEach(
    (part, i) =>
      (spec += part + (newInputs[i] ? " %'" + newInputs[i] + "' " : ""))
  );
  this.spec = spec.trim();
...
CustomBlockDefinition.prototype.addInputs = function (count) {
  ...
  this.spec = this.parseSpec(this.spec)
    .concat(inputNames.slice(-count).map((str) => "%" + str))
    .join(" ")
    .trim();
  ...
  inputNames
    .slice(-count)
    .forEach((name) => this.declarations.set(name, ["%s"]));
};
...
CustomBlockDefinition.prototype.scriptsModel = function () {
  ...
  this.scripts.forEach((element) => {
    ...
    if (block instanceof BlockMorph) {
      block.allComments().forEach((comment) => comment.align(block));
    }
  });
  proto.allComments().forEach((comment) => comment.align(proto));
...
CustomBlockDefinition.prototype.collectDependencies = function (
  ...
  if (!this.isGlobal && !localReceiver) {
    throw new Error(
      "cannot collect dependencies for local\n" +
        "custom blocks of an unspecified sprite"
    );
  }
  excluding.push(this);
  this.scripts
    .concat(this.body ? [this.body.expression] : [])
    .forEach((script) => {
      script.forAllChildren((morph) => {
        ...
        if (morph.isCustomBlock) {
          ...
          if (!contains(excluding, def) && !contains(result, def)) {
            result.push(def);
            def.collectDependencies(excluding, result, localReceiver);
          }
        }
      });
    });
...
CustomBlockDefinition.prototype.isSending = function (message, receiverName) {
  return this.scripts
    ...
    .some(
      (script) =>
        script instanceof BlockMorph && script.isSending(message, receiverName)
    );
};
...
CustomCommandBlockMorph.prototype.initializeVariables = function (oldVars) {
  ...
  this.definition.variableNames.forEach((name) => {
    ...
    this.variables.addVar(name, v instanceof Variable ? v.value : null);
  });
};

CustomCommandBlockMorph.prototype.refresh = function (aDefinition) {
  var def = aDefinition || this.definition,
    newSpec = this.isPrototype ? def.spec : def.localizedSpec(),
    oldInputs;
  ...
  if (this.blockSpec !== newSpec) {
    ...
    // update all input slots' drop-downs
...
CustomCommandBlockMorph.prototype.restoreInputs = function (oldInputs) {
  ...
  for (i = 0; i < len; i += 1) {
    ...
    if (
      old instanceof ReporterBlockMorph &&
      inp &&
      ...
      old instanceof BooleanSlotMorph &&
      inp instanceof BooleanSlotMorph
    ) {
      inp.setContents(old.evaluate());
    } else if (
      old instanceof TemplateSlotMorph &&
      inp instanceof TemplateSlotMorph
    ) {
      inp.setContents(old.evaluate());
...
CustomCommandBlockMorph.prototype.refreshPrototype = function () {
  ...
  this.parts().forEach((part) => {
    if (!part.fragment.isDeleted) {
      ...
      if (part.fragment.type) {
        // marked as input, take label as is
        frags.push(part.fragment);
      } else {
        // not an input, devide into several non-input fragments
        words = myself.definition.parseSpec(part.fragment.labelString);
        words.forEach((word) => {
  ...
  if (
    ...
    if (hat.type === "command") {
      myself = new CustomCommandBlockMorph(this.definition, true);
      hat.replaceInput(this, myself);
    } else {
  ...
  myself.parts().forEach((part) => {
    if (!(part instanceof BlockLabelPlaceHolderMorph)) {
      if (frags[i]) {
        // don't delete the default fragment
...
CustomCommandBlockMorph.prototype.declarationsFromFragments = function () {
  ...
  this.parts().forEach((part) => {
    if (part instanceof BlockInputFragmentMorph) {
      ans.set(part.fragment.labelString, [
        part.fragment.type,
...
CustomCommandBlockMorph.prototype.edit = function () {
  ...
  if (this.isPrototype) {
    ...
    if (!this.isGlobal) {
      if (contains(Object.keys(rcvr.inheritedBlocks()), this.blockSpec)) {
  ...
  if (this.isPrototype) {
    ...
    if (this.isTemplate) {
      ...
      if (this.isGlobal) {
          ...
          addOption(
            ...
            function () {
              ...
              rcvr.customBlocks.push(
                rcvr.getMethod(myself.blockSpec).copyAndBindTo(rcvr)
              );
              if (ide) {
        ...
        } else if (rcvr.exemplar && rcvr.exemplar.getMethod(this.blockSpec)) {
          // shadowed
          addOption(
            "inherited",
      ...
      menu.addItem("duplicate block definition...", "duplicateBlockDefinition");
      menu.addItem(
      ...
      if (
        this.isGlobal ||
        contains(Object.keys(rcvr.ownBlocks()), this.blockSpec)
      ) {
        menu.addItem("delete block definition...", "deleteBlockDefinition");
      }
    }

    this.variables.names().forEach((vName) => monitor(vName));
  }
...
CustomCommandBlockMorph.prototype.exportBlockDefinition = function () {
  var rcvr = this.scriptTarget(),
    ide = rcvr.parentThatIsA(IDE_Morph),
    def =
      this.isGlobal || this instanceof PrototypeHatBlockMorph
        ? this.definition
  ...
  new DialogBoxMorph(
    ...
    () => {
      ...
      if (method.isGlobal) {
        ...
        if (method) {
          rcvr
            .allDependentInvocationsOf(this.blockSpec)
...
CustomCommandBlockMorph.prototype.alternatives = function () {
  ...
  return allDefs.filter(
    (each) => each !== this.definition && each.type === type
  );
};
...
JaggedBlockMorph.prototype.drawEdges = function (ctx) {
  ...
  if (!this.cSlots().length) {
    // omit right jagged outline for c-slots
    y = 0;
    for (i = 0; i < jags; i += 1) {
...
BlockDialogMorph.prototype.init = function (target, action, environment) {
  ...
  BlockDialogMorph.uber.init.call(this, target, action, environment);
...
BlockDialogMorph.prototype.addCategoryButton = function (category) {
  ...
  button = new ToggleButtonMorph(
    ...
    () => {
      ...
      this.categories.refresh();
      if (this.types) {
        this.types.children.forEach((each) => each.setColor(colors[2]));
      }
      this.edit();
    },
  ...
  this.categories.children.forEach((button, i) => {
    ...
    button.setPosition(
      new Point(
        ...
        t +
          ((row + 1) * yPadding + row * buttonHeight + border) +
          (i > 9 ? border / 2 : 0)
      )
    );
  });
...
BlockDialogMorph.prototype.fixLayout = function () {
  ...
  if (this.body) {
    ...
    if (this.categories) {
      ...
      this.bounds.setHeight(
        this.height() + this.categories.height() + this.padding
      );
    }
  } else if (this.head) {
    // when changing an existing prototype
    if (this.types) {
      this.types.fixLayout();
      this.bounds.setWidth(
        Math.max(this.types.width(), this.head.width()) + this.padding * 2
      );
    } else {
      this.bounds.setWidth(
        Math.max(this.categories.width(), this.head.width()) + this.padding * 2
      );
    }
  ...
  if (this.types) {
    this.types.fixLayout();
    this.bounds.setHeight(this.height() + this.types.height() + this.padding);
    this.bounds.setWidth(
      Math.max(this.width(), this.types.width() + this.padding * 2)
    );
  ...
  if (this.scopes) {
    ...
    this.bounds.setHeight(
      this.height() + this.scopes.height() + this.padding / 3
    );
    this.bounds.setWidth(
      Math.max(this.width(), this.scopes.width() + this.padding * 2)
    );
...
BlockDialogMorph.prototype.accept = function () {
  if (
    this.body instanceof InputFieldMorph &&
...
BlockEditorMorph.prototype.popUp = function () {
  ...
  if (world) {
    ...
    this.handle = new HandleMorph(this, 280, 220, this.corner, this.corner);
...
BlockEditorMorph.prototype.accept = function (origin) {
  ...
  if (origin instanceof CursorMorph) {
    return;
  }
  if (this.action) {
...
BlockEditorMorph.prototype.refreshAllBlockInstances = function (oldSpec) {
  ...
  if (def.isGlobal) {
    ...
    this.target
      ...
      .forEach((context) => {
        if (
          context.expression.isCustomBlock &&
    ...
    this.target
      .parentThatIsA(StageMorph)
      .allContextsInvoking(def.blockSpec(), this.target)
      .forEach((context) => {
        if (
          context.expression.isCustomBlock &&
          context.expression.isUnattached() &&
...
BlockEditorMorph.prototype.setInitialDimensions = function () {
  ...
  this.setExtent(
    this.body.contents
      .extent()
      .add(new Point(this.padding, this.padding + th + bh))
      .min(mex)
  );
...
PrototypeHatBlockMorph.prototype.init = function (definition) {
  ...
  if (definition.variableNames.length) {
    ...
    this.add(vars);
    definition.variableNames.forEach((name) => vars.addInput(name));
  }
...
BlockLabelFragment.prototype.isSingleInput = function () {
  return !this.isMultipleInput() && this.type !== "%upvar";
};
...
*/
  var frag = this.fragment.copy(),
    ...
    isOnlyElement = this.parent.parseSpec(this.parent.blockSpec).length < 2;
...
BlockLabelPlaceHolderMorph.prototype.fixLayout = function () {
  ...
  this.bounds.setWidth(
    this.isHighlighted || !this.plainLabel
      ? h / 2
...
BlockLabelPlaceHolderMorph.prototype.render = function (ctx) {
  ...
  if (this.isHighlighted) {
    ...
    ctx.beginPath();
    ctx.arc(cx, cy, r, radians(0), radians(360), false);
    ctx.closePath();
    ctx.fill();
  }
...
InputSlotDialogMorph.prototype.createTypeButtons = function () {
  ...
  arrow = new ArrowMorph("right", PushButtonMorph.prototype.fontSize + 4, 2);
...
InputSlotDialogMorph.prototype.getInput = function () {
  ...
  if (lbl) {
    ...
    if (contains(["%b", "%boolUE"], this.fragment.type)) {
      this.fragment.defaultValue = this.slots.defaultSwitch.evaluate();
    } else {
...
InputSlotDialogMorph.prototype.fixLayout = function () {
  ...
  // label
  this.label.setLeft(left + this.padding + (maxWidth - this.label.width()) / 2);
  this.label.setTop(this.top() + (th - this.label.height()) / 2);

  // types
  this.types.fixLayout();
  this.types.setTop(this.body.bottom() + this.padding);
  this.types.setLeft(left + this.padding + (maxWidth - this.types.width()) / 2);

  // slots
  this.slots.setPosition(
    new Point(this.left() + this.padding, this.types.bottom() + this.padding)
  );
  this.slots.children.forEach((c) => c.refresh());
...
InputSlotDialogMorph.prototype.symbolMenu = function () {
  ...
  SymbolMorph.prototype.names.forEach((sym) =>
    symbols.push([
      [new SymbolMorph(sym, this.fontSize, symbolColor), localize(sym)],
      "$" + sym,
    ])
  );
...
InputSlotDialogMorph.prototype.createSlotTypeButtons = function () {
  ...
  defLabel.refresh = () => {
    if (
      this.isExpanded &&
      contains(
        ["%s", "%n", "%txt", "%anyUE", "%b", "%boolUE", "%mlt", "%code"],
        this.fragment.type
      )
    ) {
      defLabel.show();
  ...
  defSwitch.refresh = () => {
    if (this.isExpanded && contains(["%b", "%boolUE"], this.fragment.type)) {
      defSwitch.show();
    } else {
      defSwitch.hide();
    }
  ...
  loopArrow = new ToggleMorph(
    ...
    null, // environment
    null, // hint
    new SymbolMorph("loop", this.fontSize * 0.7, WHITE).getImage(),
    null // builder method that constructs the element morph
  );
  loopArrow.refresh = () => {
    ToggleMorph.prototype.refresh.call(loopArrow);
    if (this.isExpanded && contains(["%cs", "%ca"], this.fragment.type)) {
      loopArrow.show();
    } else {
      loopArrow.hide();
    }
...
InputSlotDialogMorph.prototype.fixSlotsLayout = function () {
  ...
  for (idx = 0; idx < size; idx += 1) {
    ...
    slots.children[idx].setPosition(new Point(cols[col], rows[row]));
  }
  ...
  for (idx = size; idx < size + 3; idx += 1) {
    slots.children[idx].setPosition(
      new Point(cols[col], rows[row + idx - size])
    );
  }
  ...
  this.slots.defaultInputField.setCenter(
    this.slots.defaultInputLabel
      .center()
      .add(
        new Point(
          this.slots.defaultInputField.width() / 2 +
            this.slots.defaultInputLabel.width() / 2 +
            5,
          0
        )
      )
  ...
  this.slots.settingsButton.setPosition(
    this.slots
      .bottomRight()
      .subtract(
        this.slots.settingsButton.extent().add(this.padding + this.slots.border)
      )
  );
  ...
  this.slots.userMenu = () => {
    if (
      ...
      var menu = new MenuMorph(this),
        ...
        off = "\u2610 ";
      menu.addItem(
        (this.fragment.hasOptions() ? on : off) + localize("options") + "...",
        "editSlotOptions"
      );
      menu.addItem(
        (this.fragment.isReadOnly ? on : off) + localize("read-only"),
        () => (this.fragment.isReadOnly = !this.fragment.isReadOnly)
      );
      menu.addLine();
      menu.addMenu(
        (this.fragment.hasSpecialMenu() ? on : off) + localize("menu"),
        this.specialOptionsMenu()
      );
      menu.addMenu(
        (contains(["%mlt", "%code"], this.fragment.type) ? on : off) +
          localize("special"),
      ...
      if (this.world().currentKey === 16) {
        // shift-key down
        menu.addMenu(
          (this.fragment.hasExtensionMenu() ? on : off) + localize("extension"),
...
InputSlotDialogMorph.prototype.specialOptionsMenu = function () {
  ...
  function addSpecialOptions(label, selector) {
    menu.addItem(
      (myself.fragment.options === selector ? on : off) + localize(label),
      selector
    );
  }
    ...
    this.blocks.forEach((definition) => {
      if (definition.category === category) {
        ...
        checkBox.setPosition(
          new Point(x, y + (checkBox.top() - checkBox.toggleElement.top()))
        );
...
BlockExportDialogMorph.prototype.popUp = function (wrrld) {
  ...
  if (world) {
    ...
    this.handle = new HandleMorph(this, 200, 220, this.corner, this.corner);
  }
};
...
BlockExportDialogMorph.prototype.dependencies = function () {
  var deps = [];
  this.blocks.forEach((def) => def.collectDependencies([], deps, def.receiver));
  return deps;
};
...
BlockExportDialogMorph.prototype.exportBlocks = function () {
  ...
  if (this.blocks.length) {
    ide.saveXMLAs(
      ...
      (ide.getProjectName() || localize("untitled")) + " " + localize("blocks")
    );
  } else {
    new DialogBoxMorph().inform(
      "Export blocks",
...
BlockImportDialogMorph.prototype.init = function (blocks, target, name) {
  ...
  // override inherited properites:
  this.labelString =
    localize("Import blocks") + (name ? ": " : "") + name || "";
  this.createLabel();
...
BlockRemovalDialogMorph.prototype.init = function (blocks, target) {
  ...
  // override inherited properites:
  this.labelString =
    localize("Remove unused blocks") + (name ? ": " : "") + name || "";
  this.createLabel();
...
BlockVisibilityDialogMorph.prototype.init = function (target) {
  ...
  // override inherited properites:
  this.labelString =
    localize("Hide blocks in palette") + (name ? ": " : "") + name || "";
  this.createLabel();
...
BlockVisibilityDialogMorph.prototype.selectUnused = function () {
  ...
  used.forEach((b) => {
    if (b.isCustomBlock) {
      uCust.push(
        b.isGlobal ? b.definition : this.target.getMethod(b.semanticSpec)
      );
...
BlockVisibilityDialogMorph.prototype.hideBlocks = function () {
  var ide = this.target.parentThatIsA(IDE_Morph);
  this.blocks.forEach((block) =>
    this.target.changeBlockVisibility(
      block,
      contains(this.selection, block),
