...
SnapExtensions.primitives.set("err_error(msg)", function (msg) {
...
SnapExtensions.primitives.set("err_reset", function (proc) {
  proc.resetErrorHandling();
});

SnapExtensions.primitives.set("err_ignore", nop);

// list utils (lst_):

SnapExtensions.primitives.set("lst_sort(list, fn)", function (data, fn, proc) {
  return proc.reportAtomicSort(data, fn);
});

SnapExtensions.primitives.set("lst_linked(list)", function (data) {
  return data.isLinked;
});

// text utils (txt_):

SnapExtensions.primitives.set("txt_lowercase(txt)", function (txt) {
  return txt.toLowerCase();
});

SnapExtensions.primitives.set("txt_indexof(sub, txt)", function (sub, txt) {
  return txt.indexOf(sub) + 1;
});

// bitwise operations

SnapExtensions.primitives.set("bit_and(a, b)", function (a, b) {
  return a & b;
});

SnapExtensions.primitives.set("bit_or(a, b)", function (a, b) {
  return a | b;
});

SnapExtensions.primitives.set("bit_xor(a, b)", function (a, b) {
  return a ^ b;
});

SnapExtensions.primitives.set("bit_not(a)", function (a) {
  return ~a;
});

SnapExtensions.primitives.set("bit_left_shift(a, b)", function (a, b) {
  return a << b;
});

SnapExtensions.primitives.set("bit_right_shift(a, b)", function (a, b) {
  return a >> b;
});
...
SnapExtensions.primitives.set("dta_analyze(list)", function (list, proc) {
  const dict = new Map();
  const result = [];
  const data = list.itemsArray();
  const len = data.length;
  let item;
  let i;
  for (i = 0; i < len; i += 1) {
    item = proc.reportIsA(data[i], "number") ? data[i].toString() : data[i];
    if (dict.has(item)) {
...
SnapExtensions.primitives.set("dta_group(list, fn)", function (data, fn, proc) {
  return proc.reportAtomicGroup(data, fn);
});

SnapExtensions.primitives.set("dta_transpose(list)", function (data, proc) {
...
SnapExtensions.primitives.set("map_zoom", function () {
  return this.parentThatIsA(StageMorph).worldMap.zoom;
});

SnapExtensions.primitives.set("map_zoom(n)", function (num) {
  this.parentThatIsA(StageMorph).worldMap.setZoom(num);
});

SnapExtensions.primitives.set("map_lon(x)", function (x) {
  return this.parentThatIsA(StageMorph).worldMap.lonFromSnapX(x);
});

SnapExtensions.primitives.set("map_lat(y)", function (y) {
  return this.parentThatIsA(StageMorph).worldMap.latFromSnapY(y);
});

SnapExtensions.primitives.set("map_view(lon, lat)", function (lon, lat) {
  this.parentThatIsA(StageMorph).worldMap.setView(lon, lat);
});

SnapExtensions.primitives.set("map_y(lat)", function (lat) {
  return this.parentThatIsA(StageMorph).worldMap.snapYfromLat(lat);
});

SnapExtensions.primitives.set("map_x(lon)", function (lon) {
  return this.parentThatIsA(StageMorph).worldMap.snapXfromLon(lon);
});

SnapExtensions.primitives.set("map_pan(x, y)", function (x, y) {
...
SnapExtensions.primitives.set("map_update", function () {
  const stage = this.parentThatIsA(StageMorph);
  stage.worldMap.extent = stage.dimensions;
  stage.worldMap.render();
});

SnapExtensions.primitives.set("map_loaded", function () {
  return !this.parentThatIsA(StageMorph).worldMap.loading;
});

SnapExtensions.primitives.set("map_costume", function () {
  return new Costume(this.parentThatIsA(StageMorph).worldMap.canvas, "map");
});

SnapExtensions.primitives.set("map_style(name)", function (name) {
  ...
  function (method, url, data, headers, proc) {
    ...
    if (!proc.httpRequest) {
      ...
      for (i = 1; i <= headers.length(); i += 1) {
        ...
        proc.httpRequest.setRequestHeader(header.at(1), header.at(2));
      }
...
SnapExtensions.primitives.set("geo_location(acc?)", function (includeAccuracy) {
  let crd = new List();
  const myself = this;
  const options = {
  ...
  function success(pos) {
    crd = new List([pos.coords.latitude, pos.coords.longitude]);
    if (includeAccuracy) {
...
SnapExtensions.primitives.set("mda_snap", function () {
  let camDialog;
  let result = false;
...
SnapExtensions.primitives.set("mda_record", function () {
  let soundRecorder;
  let result = false;
  soundRecorder = new SoundRecorderDialogMorph(function (audio) {
    if (audio) {
...
SnapExtensions.primitives.set("db_getall", function () {
...
SnapExtensions.primitives.set("db_remove(key)", function (key, proc) {
  proc.assertType(key, ["text", "number"]);
  window.localStorage.removeItem("-snap-project-" + key);
});

SnapExtensions.primitives.set("db_get(key)", function (key) {
  const str = window.localStorage;
  const result = str.getItem("-snap-project-" + key);
  if (!result) {
    return false;
  }
  return result;
...
SnapExtensions.primitives.set("var_delete(name)", function (name, proc) {
  let local;
  proc.assertType(name, "text");
  if (name === "") {
    return;
...
SnapExtensions.primitives.set("var_get(name)", function (name, proc) {
  proc.assertType(name, "text");
  return proc.homeContext.variables.getVar(name);
});

SnapExtensions.primitives.set("var_set(name, val)", function (name, val, proc) {
  let local;
  proc.assertType(name, "text");
  if (name === "") {
    return;
...
SnapExtensions.primitives.set("var_show(name)", function (name, proc) {
  proc.doShowVar(
    name,
...
SnapExtensions.primitives.set("var_hide(name)", function (name, proc) {
  proc.doHideVar(
    name,
    proc.context.isInCustomBlock()
      ? proc.homeContext
...
SnapExtensions.primitives.set("ide_hide(block)", function (context, proc) {
  proc.assertType(context, ["command", "reporter", "predicate"]);
  this.changeBlockVisibility(context.expression, true);
});

SnapExtensions.primitives.set("ide_show(block)", function (context, proc) {
  proc.assertType(context, ["command", "reporter", "predicate"]);
...
SnapExtensions.primitives.set("ide_translate(text)", function (text, proc) {
  if (proc.enableHyperOps) {
    if (text instanceof List) {
      return text.map((each) =>
        SnapExtensions.primitives.get("ide_translate(text)")(each, proc)
      );
    }
  }
...
SnapExtensions.primitives.set("ide_translateback(text)", function (text, proc) {
  let dict;
  if (proc.enableHyperOps) {
    if (text instanceof List) {
      return text.map((each) =>
        SnapExtensions.primitives.get("ide_translateback(text)")(each, proc)
      );
    }
  }
  dict = SnapTranslator.dict[SnapTranslator.language];
  proc.assertType(text, "text");
  return detect(Object.keys(dict), (key) => dict[key] === text) || text;
});

SnapExtensions.primitives.set("ide_language()", function () {
...
SnapExtensions.primitives.set(
  ...
  function (lang, msg, proc) {
    const ide = this.parentThatIsA(IDE_Morph);
    const disabled = ["receiveGo", "receiveCondition", "receiveMessage"];
    const flag = ide.isAppMode;
    const restoreMode = () => ide.toggleAppMode(flag);
    let callback = restoreMode;
...
SnapExtensions.primitives.set("ide_translations()", function () {
  return new List(
    SnapTranslator.languages().map(
      (lang) => new List([SnapTranslator.languageName(lang), lang])
    )
  );
});
...
SnapExtensions.primitives.set("clr_rgba(r, g, b, a)", function (r, g, b, a) {
  return new Color(r, g, b, a);
});

SnapExtensions.primitives.set("clr_channel(clr, rgba)", function (clr, rgba) {
...
SnapExtensions.primitives.set("clr_hsv(clr)", function (clr) {
  return new List(clr.hsv());
});

SnapExtensions.primitives.set("clr_hsv(h, s, v)", function (h, s, v) {
  const c = new Color();
  c.set_hsv(h, s, v);
  return c;
});

SnapExtensions.primitives.set("clr_hsl(clr)", function (clr) {
  return new List(clr.hsl());
});

SnapExtensions.primitives.set("clr_hsl(h, s, l)", function (h, s, l) {
  const c = new Color();
  c.set_hsl(h, s, l);
  return c;
});

SnapExtensions.primitives.set("clr_setpen(clr)", function (clr) {
  this.setColor(clr);
});

SnapExtensions.primitives.set("clr_pen", function () {
  return this.color;
});
...
SnapExtensions.primitives.set("srl_close(port)", function (port, proc) {
  let acc = proc.context.accumulator;
...
SnapExtensions.primitives.set("srl_read(port)", function (port, proc) {
  const acc = { result: false };
  if (!port?.readable) {
...
SnapExtensions.primitives.set("src_load(url)", function (url, proc) {
  let scriptElement;
...
SnapExtensions.menus.set(
  ...
  function () {
    const menuName = this.parent.inputs()[0].evaluate(); // first slot
    let output;
    const menus = {
      "color number": [
        "0 black=0",
