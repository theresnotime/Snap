...
List.prototype.put = function (element, index) {
  ...
  var idx = Math.round(+index) || 0,
    data = element === 0 ? 0 : element === false ? false : element || null;
...
List.prototype.map = function (callback) {
  return new List(this.itemsArray().map(callback));
};
...
List.prototype.query = function (indices) {
  ...
  if (first instanceof List) {
    select = first.isEmpty() ? this.range(1, this.length()) : first;
  } else {
    select = new List([first]);
  }
  return select
    .map((i) => this.at(i))
    .map((e) => (e instanceof List ? e.query(indices.cdr()) : e));
};
...
List.prototype.slice = function (indices) {
  ...
  return select
    .map((i) => this.at(i))
    .map((e) => (e instanceof List ? e.slice(indices.cdr()) : e));
};
...
List.prototype.range = function (start, end) {
  ...
  return new List(
    [...Array(Math.abs(end - start) + 1)].map((e, i) =>
...
List.prototype.strideTranspose = function () {
  ...
  function newIndex(old, os, ns) {
    ...
    return foo * os.at(1) + newIndex(old % ns.at(1), os.cdr(), ns.cdr());
  }
  ...
  for (i = 1; i <= oldFlat.length(); i += 1) {
    newFlat.put(oldFlat.at(i), newIndex(i - 1, oldSizes, newSizes) + 1);
  }
  ...
  function objectify(list) {
    ...
    if (canBeObject(items)) {
      items.forEach((pair) => {
        ...
        obj[pair.at(1)] = value instanceof List ? objectify(value) : value;
      });
      return obj;
    }
    return items.map((element) =>
      element instanceof List ? objectify(element) : element
    );
  }
...
List.prototype.canBeTXT = function () {
  return this.itemsArray().every(
    (item) => isString(item) || typeof item === "number"
  );
};
...
List.prototype.hasOnlyAtomicData = function () {
  return this.itemsArray().every((value) => {
    return (!isNaN(+value) && typeof value !== "boolean") || isString(value);
  });
};
...
List.prototype.blockify = function (limit = 500, count = [0]) {
  ...
  for (i = 0; i < len && count[0] < limit; i += 1) {
    ...
    if (value instanceof List) {
      slots.replaceInput(slots.addInput(), value.blockify(limit, count));
    } else if (typeof value === "boolean") {
      bool = SpriteMorph.prototype.blockForSelector("reportBoolean");
      bool.inputs()[0].setContents(value);
      bool.isDraggable = true;
      slots.replaceInput(slots.addInput(), bool);
    } else {
...
ListWatcherMorph.prototype.init = function (list, parentCell) {
  ...
  this.setExtent(
    new Point(80, 70).multiplyBy(SyntaxElementMorph.prototype.scale)
  );
  this.add(this.label);
  this.add(this.frame);
  if (!readOnly) {
...
ListWatcherMorph.prototype.update = function (anyway) {
  ...
  this.start = Math.max(
    Math.min(
      this.start,
      Math.floor((this.list.length() - 1) / this.range) * this.range + 1
    ),
    1
  );
...
ListWatcherMorph.prototype.startIndexMenu = function () {
  var i,
    ...
    menu = new MenuMorph((idx) => this.setStartIndex(idx), null, this);
  menu.addItem("1...", 1);
  for (i = 1; i < items; i += 1) {
    range = i * 100 + 1;
