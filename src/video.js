...
VideoMotion.prototype.getStageMotion = function () {
  ...
  for (i = this.winSize + 1; i < hmax; i += winStep) {
    for (j = this.winSize + 1; j < wmax; j += winStep) {
      ...
      for (
        ;
        address <= maxAddress;
        address += this.width - winStep, nextAddress += this.width
      ) {
        for (; address <= nextAddress; address += 1) {
          ...
          // below.
          gradY =
            this.curr[address - this.width] - this.curr[address + this.width];
  ...
  if (this.motionAmount > this.threshold) {
    this.motionDirection = (
      ((Math.atan2(vv, uu) * this.toDegree + 270) % 360) -
      180
...
VideoMotion.prototype.getLocalMotion = function (aSprite) {
  ...
  if (aSprite.frameNumber !== this.frameNumber) {
    ...
    ymin = Math.max(Math.floor((aSprite.top() - stage.top()) / stage.scale), 0);
    xmax = Math.min(cb.sw + xmin, stage.dimensions.x);
    ymax = Math.min(cb.sh + ymin, stage.dimensions.y - 1);
    // This is a performance critical math region.
    pixel = cb.sy * spriteWidth + cb.sx;
    for (i = ymin; i < ymax; i++, pixel += spriteWidth - cb.sw) {
      //rows
      for (j = xmin; j < xmax; j++, ++pixel) {
        //cols
        if (
          j > 0 &&
          j < this.width &&
    ...
    aSprite.motionAmount = Math.round(
      localAmountScale * activePixelNum * Math.hypot(vector.u, vector.v)
    );
    ...
    if (aSprite.motionAmount > localThreshold) {
      ...
      aSprite.motionDirection = (
        ((Math.atan2(vector.v, vector.u) * this.toDegree + 270) % 360) -
        180
      ).toFixed(2);
    }
  ...
  function getClippedBounds(sprite) {
    ...
    // Clipping X
    if (sprite.left() < stage.left()) {
      // sprite outer left stage
      bounds.sw = Math.max(
    ...
    if (sprite.right() > stage.right()) {
      // sprite outer right stage
      bounds.sw = Math.max(
