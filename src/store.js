...
XML_Serializer.prototype.store = function (object, mediaID) {
  ...
  if (this.isCollectingMedia && object[this.mediaDetectionProperty]) {
    ...
    return this.format('<ref mediaID="@"></ref>', object[this.mediaIdProperty]);
  }
  ...
  return object
    .toXML(this, mediaID)
    .replace("~", this.format('id="@"', object[this.idProperty]));
};
...
XML_Serializer.prototype.mediaXML = function () {
  ...
  this.media.forEach((object) => {
    var str = object
      .toXML(this)
      .replace("~", this.format('mediaID="@"', object[this.mediaIdProperty]));
    xml = xml + str;
  });
  return xml + "</media>";
};
...
XML_Serializer.prototype.addMedia = function (object, mediaID) {
  ...
  if (object[this.mediaIdProperty]) {
    // already present
    return -1;
  }
...
SnapSerializer.prototype.loadSpritesModel = function (xmlNode, ide) {
  ...
  model.childrenNamed("sprite").forEach((model) => {
    ...
    sprite.rotationStyle = parseFloat(model.attributes.rotation || "1");
  ...
  scene.stage.children.forEach((sprite) => {
    ...
    if (sprite.inheritanceInfo) {
      // only sprites can inherit
      exemplar = scene.spritesDict[sprite.inheritanceInfo.exemplar];
      if (exemplar) {
        sprite.setExemplar(exemplar);
      }
    }
...
SnapSerializer.prototype.loadInheritanceInfo = function (object, model) {
  ...
  if (info) {
    ...
    if (delegated) {
      object.inheritanceInfo.delegated = this.loadValue(delegated).asArray();
    }
...
SnapSerializer.prototype.loadCostumes = function (object, model) {
  ...
  var costumes = model.childNamed("costumes"),
    costume;
  if (costumes) {
    object.costumes = this.loadValue(
      costumes.require("list", function () {
        console.log(
          object.name + ": missing required costumes list, " + "improvising..."
        );
...
SnapSerializer.prototype.loadSounds = function (object, model) {
  ...
  if (sounds) {
    ...
    object.sounds = this.loadValue(
      sounds.require("list", function () {
        console.log(
          object.name + ": missing required sounds list, " + "improvising..."
        );
        return new XML_Element("list");
      })
    );
...
SnapSerializer.prototype.loadVariables = function (varFrame, element, object) {
  ...
  element.children.forEach((child) => {
    ...
    v.value = v.isTransient || !value ? 0 : this.loadValue(value, object);
...
SnapSerializer.prototype.loadCustomBlocks = function (
  ...
  element.children.forEach((child) => {
    ...
    if (!SpriteMorph.prototype.allCategories().includes(definition.category)) {
    ...
    names = definition
      .parseSpec(definition.spec)
      .filter((str) => str.charAt(0) === "%" && str.length > 1)
    ...
    if (inputs) {
      ...
      inputs.children.forEach((child) => {
        ...
        i += 1;
        definition.declarations.set(names[i], [
    ...
    if (vars) {
      definition.variableNames = this.loadValue(vars.require("list")).asArray();
    }
...
SnapSerializer.prototype.loadBlock = function (model, isReporter, object) {
  ...
  if (model.tag === "block") {
    if (Object.prototype.hasOwnProperty.call(model.attributes, "var")) {
      block = SpriteMorph.prototype.variableBlock(model.attributes["var"]);
    } else {
    ...
    if (isGlobal) {
      ...
      if (!info && this.scene.targetStage) {
        // importing block files
        info = detect(
    ...
    block =
      info.type === "command"
        ? new CustomCommandBlockMorph(info, false)
        : new CustomReporterBlockMorph(info, info.type === "predicate", false);
  }
  if (block === null) {
  ...
  switch (model.tag) {
      ...
      if (el) {
        ...
        if (el) {
          ...
          if (el) {
            ...
            if (bool) {
              v.expression = new BooleanSlotMorph(this.loadValue(bool));
            } else {
    ...
    case "costume":
      ...
      if (Object.prototype.hasOwnProperty.call(model.attributes, "image")) {
        image = new Image();
        if (
          model.attributes.image.indexOf("data:image/svg+xml") === 0 &&
          ...
          image.onload = function () {
            ...
            v.version = +new Date();
            if (
              Object.prototype.hasOwnProperty.call(model.attributes, "embed")
            ) {
...
Array.prototype.toXML = function (serializer) {
  return this.reduce((xml, item) => xml + serializer.store(item), "");
};
...
Scene.prototype.toXML = function (serializer) {
  ...
  function code(key) {
    var str = "";
    Object.keys(StageMorph.prototype[key]).forEach((selector) => {
      str +=
        "<" +
        selector +
        ">" +
        XML_Element.prototype.escape(StageMorph.prototype[key][selector]) +
        "</" +
        selector +
        ">";
    });
  ...
  xml = serializer.format(
    ...
    this.unifiedPalette && !this.showCategories ? ' categories="false"' : "",
    this.unifiedPalette && !this.showPaletteButtons ? ' buttons="false"' : "",
    this.disableClickToRun ? ' clickrun="false"' : "",
    this.penColorModel === "hsl" ? ' colormodel="hsl"' : "",
    this.notes || "",
    serializer.paletteToXML(this.customCategories),
    Object.keys(this.hiddenPrimitives).reduce((a, b) => a + " " + b, ""),
    code("codeHeaders"),
    code("codeMappings"),
...
StageMorph.prototype.toXML = function (serializer) {
  ...
  return serializer.format(
    ...
    serializer.root.sprites.asArray().indexOf(serializer.root.currentSprite) +
      1,
...
ReporterBlockMorph.prototype.toXML = function (serializer) {
  if (this.selector === "reportGetVar") {
    if (!this.comment) {
      return serializer.format('<block var="@"/>', this.blockSpec);
    } else {
      return serializer.format(
        '<block var="@">%</block>',
        this.blockSpec,
...
CustomCommandBlockMorph.prototype.toBlockXML = function (serializer) {
  ...
  return serializer.format(
    ...
    this.semanticSpec,
    this.isGlobal ? "" : serializer.format(' scope="@"', scope),
    serializer.store(this.inputs()),
    this.isGlobal &&
      this.definition.variableNames.length &&
      !serializer.isExportingBlocksLibrary
      ? "<variables>" + this.variables.toXML(serializer) + "</variables>"
      : "",
    this.comment ? this.comment.toXML(serializer) : ""
  );
};
    ...
    Array.from(this.declarations.keys()).reduce((xml, decl) => {
      ...
      return (
        ...
        serializer.format(
          ...
          this.declarations.get(decl)[3] ? ' readonly="true"' : "",
...
InputSlotMorph.prototype.toXML = function (serializer) {
  if (this.selectedBlock) {
    return serializer.format(
      ...
      this.selectedBlock instanceof CommandBlockMorph
        ? "command"
        : this.selectedBlock.isPredicate
        ? "predicate"
        : "reporter",
