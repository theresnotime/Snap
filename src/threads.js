...
function snapEquals(a, b) {
  ...
  if (
    ...
    [a, b].some(
      (any) =>
        contains(NONNUMBERS, any) || (isString(any) && any.indexOf(" ") > -1)
    )
  ) {
    x = a;
    y = b;
...
function invoke(
  ...
  if (action instanceof Context) {
    ...
    proc.context = new Context(null, action.blockSequence(), proc.homeContext);
  ...
  while (proc.isRunning()) {
    if (deadline && Date.now() > deadline) {
      throw new Error(
        localize(timeoutErrorMsg || "a synchronous Snap! script has timed out")
      );
    }
...
ThreadManager.prototype.startProcess = function (
  ...
  if (variables instanceof VariableFrame) {
    Object.keys(variables.vars).forEach(
      (vName) =>
...
ThreadManager.prototype.stopAllForBlock = function (aTopBlock) {
  this.processesForBlock(aTopBlock, true).forEach((proc) => proc.stop());
};
...
ThreadManager.prototype.isPaused = function () {
  return detect(this.processes, (proc) => proc.isPaused) !== null;
};
  ...
  this.processes.forEach((proc) => {
    ...
    if ((!proc.isRunning() && !proc.errorFlag) || proc.isDead) {
      if (proc.topBlock instanceof BlockMorph) {
        ...
        if (count) {
          glow = proc.topBlock.getHighlight() || proc.topBlock.addHighlight();
        ...
        if (proc.onComplete instanceof Function) {
          ...
          if (result instanceof List) {
            proc.topBlock.showBubble(
              result.isTable()
                ? new TableFrameMorph(new TableMorph(result, 10))
                : new ListWatcherMorph(result),
              proc.exportResult,
              proc.receiver
            );
          } else {
            proc.topBlock.showBubble(result, proc.exportResult, proc.receiver);
          }
        }
...
ThreadManager.prototype.processesForBlock = function (block, only) {
  ...
  return this.processes.filter(
    (each) => each.topBlock === top && each.isRunning() && !each.isDead
  );
};

ThreadManager.prototype.doWhen = function (block, receiver, stopIt) {
  if (this.pauseCustomHatBlocks) {
    return;
  }
...
Process.prototype.evaluateContext = function () {
  ...
  if (exp instanceof Variable) {
    // special case for empty reporter rings
...
Process.prototype.evaluateBlock = function (block, argCount) {
  ...
  if (argCount > inputs.length) {
    ...
    if (this.flashContext()) {
      return;
    } // yield to flash the block
    if (this[selector]) {
      rcvr = this;
    }
    if (this.isCatchingErrors) {
      try {
        this.returnValueToParentContext(rcvr[selector].apply(rcvr, inputs));
        this.popContext();
      } catch (error) {
        this.handleError(error, block);
      }
...
Process.prototype.reportApplyExtension = function (prim, args) {
  ...
  return ext.apply(this.blockReceiver(), args.itemsArray().concat([this]));
};
...
Process.prototype.doReport = function (block) {
  ...
  if (this.flashContext()) {
    return;
  } // flash the block here, special form
...
Process.prototype.evaluateInput = function (input) {
  ...
  if (this.flashContext()) {
    return;
  } // yield to flash the current argMorph
  if (input.bindingID) {
    if (this.isCatchingErrors) {
      try {
    ...
    if (ans) {
      if (
        input.constructor === CommandSlotMorph ||
        input.constructor === ReporterSlotMorph ||
        (input instanceof CSlotMorph && (!input.isStatic || input.isLambda))
      ) {
    ...
    if (exp.isUnevaluated) {
      ...
      if (
        ...
        if (exp.bindingID) {
          ...
          if (ans) {
            if (
              exp.constructor === CommandSlotMorph ||
              exp.constructor === ReporterSlotMorph ||
              (exp instanceof CSlotMorph && (!exp.isStatic || exp.isLambda))
            ) {
              ans = this.reify(ans, new List());
            }
          }
...
Process.prototype.reify = function (topBlock, parameterNames, isCustomBlock) {
  ...
  if (topBlock) {
    context.expression =
      this.enableLiveCoding || this.enableSingleStepping
        ? topBlock
        : topBlock.fullCopy();
  ...
  context.receiver = this.context ? this.context.receiver : this.receiver;
...
Process.prototype.reportJSFunction = function (parmNames, body) {
  ...
  return Function.apply(null, parmNames.itemsArray().concat([body]));
};
...
Process.prototype.initializeFor = function (context, args) {
  ...
  if (context.isContinuation) {
    throw new Error("continuations cannot be forked");
  }
  if (!(context instanceof Context)) {
    throw new Error(
  ...
  var outer = new Context(null, null, context.outerContext),
    runnable = new Context(null, context.expression, outer),
    parms = args.itemsArray(),
    i,
    value;
...
Process.prototype.doDeclareVariables = function (varNames) {
  ...
  varNames.itemsArray().forEach((name) => varFrame.addVar(name));
};
...
Process.prototype.doTellTo = function (sprite, context, args) {
  this.doRun(this.reportAttributeOf(context, sprite), args);
};

Process.prototype.reportAskFor = function (sprite, context, args) {
  this.evaluate(this.reportAttributeOf(context, sprite), args);
};
...
Process.prototype.reportListAttribute = function (choice, list) {
  ...
  switch (option) {
    ...
    case "csv":
      ...
      throw new Error(localize("unable to convert to") + " " + localize("CSV"));
    case "json":
      this.assertType(list, "list");
      if (list.canBeJSON()) {
        return list.asJSON();
...
Process.prototype.concatenateLinkedLists = function (lists) {
  ...
  return lists.cons(
    first.at(1),
    this.concatenateLinkedLists(lists.cons(first.cdr(), lists.cdr()))
  );
};
  ...
  if (inputs[0] instanceof List && this.enableHyperOps) {
    ...
    if (this.context.accumulator === null) {
      ...
      if (!trueIsBlock && !falseIsBlock) {
        this.returnValueToParentContext(
          inputs[0].deepMap((leaf) =>
            leaf
              ? this.context.accumulator.trueCase
    ...
    if (accumulator.results.length === inputs[0].length()) {
      ...
      this.returnValueToParentContext(new List(accumulator.results));
      this.popContext();
      return;
    }
...
Process.prototype.reportGlobalFlag = function (name) {
  ...
  switch (name) {
    ...
    case "video capture":
      return (
        !isNil(stage.projectionSource) &&
        stage.projectionLayer().getContext("2d").getImageData(0, 0, 1, 1)
          .data[3] > 0
      );
    case "mirror video":
...
Process.prototype.reportMap = function (reporter, list) {
  ...
  if (list.isLinked) {
    if (this.context.accumulator === null) {
      ...
      this.context.accumulator.end.rest = list.cons(this.context.inputs.pop());
      this.context.accumulator.end = this.context.accumulator.end.rest;
      this.context.accumulator.idx += 1;
      this.context.accumulator.remaining -= 1;
    }
    if (this.context.accumulator.remaining === 0) {
      this.context.accumulator.end.rest = list
        .cons(this.context.inputs[2])
        .cdr();
      this.returnValueToParentContext(this.context.accumulator.target.cdr());
      return;
    }
    ...
    if (this.context.accumulator.length === list.length()) {
      this.returnValueToParentContext(new List(this.context.accumulator));
      return;
    }
...
Process.prototype.reportFindFirst = function (predicate, list) {
  ...
  if (list.isLinked) {
    if (this.context.accumulator === null) {
      ...
      if (this.context.inputs.pop() === true) {
        this.returnValueToParentContext(this.context.accumulator.source.at(1));
        return;
      }
      this.context.accumulator.remaining -= 1;
      this.context.accumulator.idx += 1;
    ...
    if (this.context.accumulator === null) {
      ...
      if (this.context.inputs.pop() === true) {
        this.returnValueToParentContext(this.context.accumulator.current);
        return;
      }
    }
    if (this.context.accumulator.idx === list.length()) {
  ...
  if (list.isLinked) {
    if (this.context.accumulator === null) {
      ...
      if (this.canRunOptimizedForCombine(reporter)) {
        return this.reportListAggregation(list, reporter.expression.selector);
      }
...
Process.prototype.doPlaySoundUntilDone = function (name) {
  ...
  if (
    name === null ||
    this.context.activeAudio.ended ||
  ...
  if (!(name instanceof List)) {
    sound =
      name instanceof Sound
        ? name
        : typeof name === "number"
        ? this.blockReceiver().sounds.at(name)
        : detect(
...
Process.prototype.reportGetSoundAttribute = function (choice, soundName) {
  var sound =
      soundName instanceof Sound
        ...
        : soundName instanceof List
        ? this.encodeSound(soundName)
        : detect(
            this.blockReceiver().sounds.asArray(),
...
Process.prototype.encodeSound = function (samples, rate) {
  ...
  if (channels === 1) {
    arrayBuffer.copyToChannel(Float32Array.from(samples.itemsArray()), 0, 0);
  } else {
...
Process.prototype.doAsk = function (data) {
  ...
  if (!this.prompter) {
    activePrompter = detect(
      stage.children,
      (morph) =>
        morph instanceof StagePrompterMorph || morph instanceof StagePickerMorph
    );
    if (!activePrompter) {
      if (data instanceof List) {
        ...
        if (isStage) {
          this.prompter.popup(
            stage,
            stage.center().subtract(this.prompter.extent().floorDivideBy(2))
          );
        } else {
          this.prompter.popup(
            stage,
            rightSpace > this.prompter.width() || rightSpace >= leftSpace
              ? rcvr.topRight()
              : rcvr.topLeft().subtract(new Point(this.prompter.width(), 0))
          );
        }
      } else {
...
Process.prototype.checkURLAllowed = function (url) {
  if (
    ["users", "logout", "projects", "collections"].some((pathPart) => {
      // Check out whether we're targeting one of the remote domains
      return Object.values(Cloud.prototype.knownDomains)
        .filter((each) => each.includes("snap"))
        .some((domain) =>
          url.match(
...
Process.prototype.doBroadcast = function (message, receivers) {
  ...
  if (target === "all") {
    ...
    rcvrs = target
      .itemsArray()
      .map((each) => this.getOtherObject(each, thisObj, stage));
  } else {
    return; // abort
  }
      ...
      if (isSnapObject(morph)) {
        morph.allHatBlocksFor(msg).forEach((block) => {
          ...
          if (block.selector === "receiveMessage") {
            ...
            procs.push(
              stage.threads.startProcess(block, morph, stage.isThreadSafe)
            );
          }
        });
      }
    });
    (stage.messageCallbacks[""] || []).forEach(
      (callback) => callback(msg) // for "any" message, pass it along as argument
    );
    (stage.messageCallbacks[msg] || []).forEach(
      (callback) => callback() // for a particular message
    );
  }
  return procs;
};
...
Process.prototype.doBroadcastAndWait = function (message, target) {
  if (!this.context.activeSends) {
    ...
    if (this.isRunning()) {
      this.context.activeSends.forEach((proc) => proc.runStep());
    }
  }
...
Process.prototype.reportLessThanOrEquals = function (a, b) {
  return this.hyperDyadic((a, b) => !this.reportBasicGreaterThan(a, b), a, b);
};
...
Process.prototype.reportGreaterThanOrEquals = function (a, b) {
  return this.hyperDyadic((a, b) => !this.reportBasicLessThan(a, b), a, b);
};
...
Process.prototype.rawParseCSV = function (text, delim) {
  ...
  records = new List(records.map((row) => new List(row)));
    ...
    if (jsonObject instanceof Array) {
      ...
      return new List(
        Object.keys(jsonObject).map(function (eachKey) {
          return new List([eachKey, listify(jsonObject[eachKey])]);
        })
      );
    } else {
      return jsonObject;
...
Process.prototype.getOtherObject = function (name, thisObj, stageObj) {
  ...
  var stage = isNil(stageObj) ? thisObj.parentThatIsA(StageMorph) : stageObj,
    thatObj = null;
  if (stage) {
    // find the corresponding sprite on the stage
    thatObj = detect(stage.children, (morph) => morph.name === name);
    if (!thatObj) {
...
Process.prototype.doFaceTowards = function (name) {
  ...
  if (thisObj) {
    if (this.inputOption(name) === "center") {
      ...
      if (name instanceof List) {
        thisObj.faceToXY(name.at(1), name.at(2));
        return;
      }
      thatObj = this.getOtherObject(name, this.homeContext.receiver);
      if (thatObj) {
        thisObj.faceToXY(thatObj.xPosition(), thatObj.yPosition());
      }
    }
  }
};
...
Process.prototype.setColorDimension = function (name, num) {
  ...
  this.blockReceiver().setColorDimension(options.indexOf(choice), +num);
};
...
Process.prototype.changeColorDimension = function (name, num) {
  ...
  this.blockReceiver().changeColorDimension(options.indexOf(choice), +num);
};

Process.prototype.setPenColorDimension = Process.prototype.setColorDimension;
...
Process.prototype.blitOn = function (name, mask, thisObj, stage) {
  ...
  if (name instanceof List) {
    // assume all elements to be sprites
...
Process.prototype.objectTouchingObject = function (thisObj, name) {
  ...
  var those, stage, box, mouse;

  if (this.inputOption(name) === "mouse-pointer") {
    mouse = thisObj.world().hand.position();
    if (
      thisObj.bounds.containsPoint(mouse) &&
    ...
    if (stage) {
      ...
      if (
        this.inputOption(name) === "pen trails" &&
      ...
      if (
        those.some(
          (any) => any.isVisible && thisObj.isTouching(any)
  ...
  return thisObj.parts.some((any) => this.objectTouchingObject(any, name));
};
...
Process.prototype.colorAtSprite = function (sprite) {
  ...
  var point =
      sprite instanceof SpriteMorph ? sprite.rotationCenter() : sprite.center(),
...
Process.prototype.reportDistanceTo = function (name) {
  ...
  if (thisObj) {
    ...
    if (this.inputOption(name) === "mouse-pointer") {
      ...
      return new Point(thisObj.xPosition(), thisObj.yPosition()).distanceTo(
        ZERO
      );
    } else if (name instanceof List) {
      return new Point(thisObj.xPosition(), thisObj.yPosition()).distanceTo(
        new Point(name.at(1), name.at(2))
      );
    }
    stage = thisObj.parentThatIsA(StageMorph);
...
Process.prototype.reportRayLengthTo = function (name) {
  ...
  hSect = (yLevel) => {
    ...
    if (
      (circa(x) === circa(rc.x) &&
        ((dir === 180 && rc.y < yLevel) || (dir === 0 && rc.y > yLevel))) ||
      (x > rc.x && dir >= 0 && dir < 180) ||
      (circa(x) < circa(rc.x) && dir >= 180 && dir < 360)
    ) {
      if (x >= left && x <= right) {
  ...
  vSect = (xLevel) => {
    ...
    if (
      (circa(y) === circa(rc.y) &&
        ((dir === 90 && rc.x < xLevel) || (dir === 270 && rc.x > xLevel))) ||
  ...
  if (targetBounds.containsPoint(rc)) {
    ...
    if (dir !== 90) {
      if (
        Math.sign(rc.x - intersections[0].x) !==
...
Process.prototype.reportDirectionTo = function (name) {
  ...
  if (thisObj) {
    ...
    if (name instanceof List) {
      return thisObj.angleToXY(name.at(1), name.at(2));
    }
    thatObj = this.getOtherObject(name, this.homeContext.receiver);
    if (thatObj) {
      return thisObj.angleToXY(thatObj.xPosition(), thatObj.yPosition());
    }
  ...
  switch (choice) {
    ...
    case "definition":
      if (expr.isCustomBlock) {
        ...
        return (
          this.blockReceiver().getMethod(expr.semanticSpec).body ||
          new Context()
        );
      }
      return new Context();
    ...
    case "type":
      return (
        ["command", "reporter", "predicate"].indexOf(this.reportTypeOf(block)) +
        1
      );
    case "scope":
...
Process.prototype.doSetBlockAttribute = function (attribute, block, val) {
    ...
    if (def.isGlobal) {
      return (
        ide.sprites
          .asArray()
          .concat([ide.stage])
          .some((any, idx) => any.usesBlockInstance(def, false, idx)) ||
        ide.stage
          .allBlockInstancesInData(def)
          .some((any) => !any.isUnattached())
      );
    }
    return rcvr
      .allDependentInvocationsOf(oldSpec)
      .some((any) => !any.isUnattached());
  }
  ...
  function isMajorTypeChange() {
    ...
    return (
      (type === "command" && rep.includes(oldType)) ||
  ...
  switch (choice) {
    ...
    case "category":
      ...
      if (this.reportTypeOf(val) === "text") {
        def.category = contains(SpriteMorph.prototype.allCategories(), val)
          ? val
          : "other";
        break;
      }
      def.category = SpriteMorph.prototype.allCategories()[+val - 1] || "other";
      break;
    case "type":
      this.assertType(val, ["number", "text"]);
      if (this.reportTypeOf(val) === "text") {
      ...
      if (
        rcvr.allBlockInstances(def).every((block) => block.isChangeableTo(type))
      ) {
        oldType = def.type;
        def.type = type;
      } else {
      ...
      if (isMajorTypeChange()) {
        ...
        if (def.isGlobal) {
          ide.stage
            .allContextsUsing(def)
            .forEach((context) => (context.expression = def.blockInstance()));
        } else {
          ide.stage
            .allContextsInvoking(def.blockSpec(), rcvr)
            .forEach((context) => (context.expression = def.blockInstance()));
        }
      }
      break;
  ...
  if (def.isGlobal) {
    ...
    ide.stage
      .allContextsInvoking(def.blockSpec(), rcvr)
      .forEach((context) => context.changed());
  }
  if (template) {
...
Process.prototype.reportBasicAttributeOf = function (attribute, name) {
  ...
  if (thisObj) {
    ...
    if (thatObj) {
      ...
      if (attribute instanceof BlockMorph) {
        // a "wish"
...
Process.prototype.reportGet = function (query) {
  ...
  function allOtherSprites() {
    var stage = thisObj.parentThatIsA(StageMorph),
      sprites = stage.children.filter(
        (each) => each instanceof SpriteMorph && each !== thisObj
      ),
  ...
  if (thisObj) {
    switch (this.inputOption(query)) {
      ...
      case "clones":
        ...
        return new List(
          allOtherSprites().filter(
            (each) => each.isTemporary && each.cloneOriginName === objName
          )
        );
      case "other clones":
        return thisObj.isTemporary ? this.reportGet(["clones"]) : new List();
      case "neighbors":
      ...
      case "scripts":
        return new List(
          thisObj.scripts.children
            .filter((each) => each instanceof BlockMorph)
            .map((each) => each.fullCopy().reify())
        );
      case "blocks": // palette unoordered without inherited methods
        return new List(
          thisObj
            .parentThatIsA(StageMorph)
            .globalBlocks.concat(thisObj.allBlocks(true))
            .filter((def) => !def.isHelper)
            .map((def) => def.blockInstance().reify())
            .concat(
              SpriteMorph.prototype.categories.reduce(
                (blocks, category) =>
                  blocks.concat(
                    thisObj
                      .getPrimitiveTemplates(category)
                      .filter(
                        (each) =>
...
Process.prototype.doSet = function (attribute, value) {
  ...
  this.assertAlive(rcvr);
  if (
    !(attribute instanceof Context || attribute instanceof Array) ||
  ...
  switch (name) {
    ...
    case "my anchor":
      ...
      if (value instanceof SpriteMorph) {
        ...
        if (!rcvr.enableNesting || contains(rcvr.allParts(), value)) {
          throw new Error(localize("unable to nest\n(disabled or circular?)"));
        }
    ...
    case "my name":
      ...
      ide = rcvr.parentThatIsA(IDE_Morph);
      if (ide) {
        rcvr.setName(ide.newSpriteName(value.toString(), rcvr));
        ide.spriteBar.nameField.setContents(ide.currentSprite.name.toString());
      }
      break;
    case "dangling?":
    case "my dangling?":
    ...
    case "microphone modifier":
      ...
      break;
    default:
      throw new Error('"' + localize(name) + '" ' + localize("is read-only"));
  }
};
  ...
  if (this.homeContext.receiver) {
    ...
    if (stage) {
      ...
      if (keyString instanceof List && this.enableHyperOps) {
        return keyString.map((each) => stage.keysPressed[each] !== undefined);
      }
...
Process.prototype.startVideo = function (stage) {
  // interpolated
  if (this.reportGlobalFlag("video capture")) {
    return;
  }
  if (!stage.projectionSource || !stage.projectionSource.stream) {
    // wait until video is turned on
...
Process.prototype.doMapValueCode = function (type, aString) {
  ...
  switch (tp) {
    ...
    case "false":
      ...
      break;
    default:
      throw new Error(localize("unsupported data type") + ': "' + tp + '"');
  }
};
...
Process.prototype.doPlayFrequency = function (hz, secs) {
  this.doPlayFrequencyForSecs(parseFloat(hz || "0"), parseFloat(secs || "0"));
};
...
Process.prototype.reportNewCostume = function (pixels, width, height, name) {
  ...
  if (width <= 0 || height <= 0) {
    throw new Error("cannot handle zero width or height");
  }
  ...
  return new Costume(
    canvas,
    name || (rcvr || this.blockReceiver()).newCostumeName(localize("costume"))
  );
};
...
Process.prototype.reportPentrailsAsSVG = function () {
  ...
  if (!this.context.accumulator) {
    ...
    if (!stage.trailsLog.length) {
      throw new Error(
        localize("there are currently no\nvectorizable pen trail segments")
      );
    }
...
Process.prototype.getVarNamed = function (name) {
  ...
  var frame =
      this.homeContext.variables.silentFind(name) ||
...
Context.prototype.toBlock = function () {
  ...
  if (this.expression instanceof Array) {
    ...
    if (block instanceof RingMorph && !block.contents()) {
      // empty ring
      return block;
    }
  ...
  if (!this.isContinuation) {
    this.inputs.forEach((inp) => ring.parts()[1].addInput(inp));
  }
  return ring;
};
...
Context.prototype.continuation = function (isReporter) {
  ...
  if (this.expression instanceof Array) {
    ...
    cont = new Context(null, isReporter ? "expectReport" : "popContext");
...
Context.prototype.copyForContinuation = function () {
  var cpy = copy(this),
    cur = cpy,
    isReporter = !(
      this.expression instanceof Array || isString(this.expression)
    );
  if (isReporter) {
...
Context.prototype.lastFlashable = function () {
  ...
  if (
    this.expression instanceof SyntaxElementMorph &&
...
Context.prototype.copyWithInputs = function (inputs) {
  return this.expression ? this.expression.copyWithInputs(inputs) : this;
};
...
VariableFrame.prototype.copy = function () {
  ...
  this.names().forEach((vName) => frame.addVar(vName, this.getVar(vName)));
  return frame;
};
...
VariableFrame.prototype.addVar = function (name, value) {
  this.vars[name] = new Variable(
    value === 0 ? 0 : value === false ? false : value === "" ? "" : value || 0
  );
};
...
*/
  var answer = [],
    each,
    dict = this.allNamesDict(upTo, includeHidden);
...
JSCompiler.prototype.compileExpression = function (block) {
  ...
  switch (selector) {
    ...
    case "reportGetVar":
      return (
        this.gensyms.get(block.blockSpec) ||
        'proc.getVarNamed("' + this.escape(block.blockSpec) + '")'
      );
    case "doSetVar":
      ...
      if (inputs[0] instanceof ArgMorph) {
        ...
        if (target) {
          return (
            ...
            ");" +
            target +
            "=isNaN(v)?d:v+parseFloat(d)}"
          );
        }
      }
      // redirect var to process
      return (
        "proc.incrementVarNamed(" +
        this.compileInput(inputs[0]) +
        ", " +
        this.compileInput(inputs[1]) +
        ")"
...
JSCompiler.prototype.escape = function (string) {
  ...
  while (len > i) {
    ...
    if (safe_chars.indexOf(char) === -1) {
      escaped +=
        "\\u" + (char.charCodeAt(0) | 0x10000).toString(16).substring(1);
    } else {
      escaped += char;
    }
  }
