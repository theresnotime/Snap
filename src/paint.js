...
PaintEditorMorph.prototype.buildToolbox = function () {
  ...
  Object.keys(tools).forEach(function (tool) {
    ...
    btn.setPosition(new Point(left + x, top + y));
    x += btn.width() + padding;
    if (tool === "crosshairs") {
      x = 0;
...
PaintEditorMorph.prototype.buildEdits = function () {
  ...
  this.edits.add(
    this.pushButton("Vector", function () {
...
PaintEditorMorph.prototype.openIn = function (
  ...
  if (this.oldim) {
    ...
    this.paper.rotationCenter = (oldrc || new Point(0, 0)).add(
      new Point(
...
PaintEditorMorph.prototype.ok = function () {
  ...
  this.callback(this.paper.paper, this.paper.rotationCenter);
  this.destroy();
};
...
PaintEditorMorph.prototype.switchToVector = function () {
  ...
  this.object.edit(this.world(), this.ide, true);
};
...
PaintCanvasMorph.prototype.floodfill = function (sourcepoint) {
  ...
  if (
    sourcecolor[0] === this.settings.primarycolor.r &&
    ...
    case "circleSolid":
      ...
      if (this.isShiftPressed()) {
        ...
        for (i = 0; i < width; i += 1) {
          mctx.lineTo(
            i,
            2 * h * Math.sqrt(2 - Math.pow((i - x) / (2 * w), 2)) + y
          );
        }
        for (i = width; i > 0; i -= 1) {
          mctx.lineTo(
            i,
            -1 * (2 * h) * Math.sqrt(2 - Math.pow((i - x) / (2 * w), 2)) + y
          );
        }
      }
      mctx.closePath();
...
PaintCanvasMorph.prototype.buildContents = function () {
  ...
  var i,
    j,
    bkctx = this.background.getContext("2d");
...
PaintCanvasMorph.prototype.rerender = PaintCanvasMorph.prototype.drawNew; // ugly hack, but hey, it works ;-) jens
...
PaintCanvasMorph.prototype.edge = InputFieldMorph.prototype.edge;

PaintCanvasMorph.prototype.fontSize = InputFieldMorph.prototype.fontSize;

PaintCanvasMorph.prototype.typeInPadding =
  InputFieldMorph.prototype.typeInPadding;

PaintCanvasMorph.prototype.contrast = InputFieldMorph.prototype.contrast;
